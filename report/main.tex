...
% -------------------------------------------------------------------
\subsection{Q1c --- Which filter weight changes the most, and why?}
% -------------------------------------------------------------------

From the computed gradient matrix:
\[
\frac{\partial L}{\partial W} = \begin{bmatrix} 0.25 & -1.25 \\ 0.5 & -0.5 \end{bmatrix}
\]

The largest magnitude belongs to $W_{12}$ with a gradient of $-1.25$. In one
step of gradient descent, $W_{12}$ would therefore change the most, and it
would \emph{increase} (since we subtract the gradient).

Structurally, $W_{12}$ is the top-right element of the filter, so during the
forward pass it multiplies the second column of each $2\times2$ patch it lands
on. Looking at the four sliding positions, the dominant contribution comes from
position $(1,1)$, where $W_{12}$ multiplied $X_{12} = 2$ --- the largest
single input value in any patch. This large activation amplifies the upstream
gradient $\delta_{11} = -0.5$, giving a contribution of $(-0.5)(2) = -1.0$
to the total. The other non-zero contribution comes from position $(2,2)$,
where $W_{12}$ saw $X_{23} = 1$, adding another $(-0.25)(1) = -0.25$.
Position $(2,1)$ would have contributed too, but $Z_{21} = -1$ was killed by
ReLU, zeroing that gradient entirely. Together, these effects push $W_{12}$'s
gradient to the largest magnitude in the filter.

% ===================================================================
\section{Q3: Pooling and Normalization}
% ===================================================================

% -------------------------------------------------------------------
\subsection{Q3a --- Max Pooling}
% -------------------------------------------------------------------

Replacing the flatten + FC layer with $2\times2$ global max pooling over $A$:
\[
A = \begin{bmatrix}2 & 2 \\ 0 & 3\end{bmatrix}
\quad \Rightarrow \quad
Y = \max(A) = 3 \quad \text{(at position } A_{22}\text{)}
\]

For the gradient flow, max pooling acts as a hard selector: only the winning
neuron receives gradient; all others get zero. Formally,
\[
\frac{\partial Y}{\partial A_{ij}} =
\begin{cases} 1 & \text{if } A_{ij} = \max(A) \\ 0 & \text{otherwise} \end{cases}
\]

So $\partial L / \partial A$ is a $2\times2$ matrix with a $1$ only at position
$(2,2)$ (scaled by $\partial L/\partial Y$), compared to the original
architecture where all four positions received gradient through their respective
weights in $V$.

Tracing back through ReLU and the convolution, $A_{22}$ was produced by the
bottom-right $2\times2$ patch of $X$: $\{X_{22}, X_{23}, X_{32}, X_{33}\}$.
Every pixel that does \emph{not} belong to this patch, or belongs only to a
position killed by ReLU (specifically $Z_{21}=-1$), has \textbf{zero gradient}.
This means six of the nine pixels in $X$ --- the entire top row
($X_{11}, X_{12}, X_{13}$), plus $X_{21}$ and $X_{31}$ --- contribute nothing
to the update.

% -------------------------------------------------------------------
\subsection{Q3b --- Layer Normalization}
% -------------------------------------------------------------------

Inserting Layer Normalization after ReLU, applied to $A$ treated as a flat
4-element vector $[2, 2, 0, 3]$ (with $\gamma=1$, $\beta=0$, $\epsilon=0$):

\[
\mu = \frac{2+2+0+3}{4} = 1.75
\]
\[
\sigma^2 = \frac{(0.25)^2 + (0.25)^2 + (-1.75)^2 + (1.25)^2}{4}
         = \frac{0.0625 + 0.0625 + 3.0625 + 1.5625}{4} = 1.1875
\]
\[
\sigma = \sqrt{1.1875} \approx 1.0898
\]

Normalizing each element $\hat{a}_i = (a_i - \mu)/\sigma$:
...
\midrule
$\frac{0.25}{1.0898} \approx +0.2294$ &
$\frac{0.25}{1.0898} \approx +0.2294$ &
$\frac{-1.75}{1.0898} \approx -1.6059$ &
$\frac{1.25}{1.0898} \approx +1.1471$ \\
\bottomrule
\end{tabular}
\end{center}

\[
A_{\text{norm}} = \begin{bmatrix} 0.2294 & 0.2294 \\ -1.6059 & 1.1471 \end{bmatrix}
\]

% -------------------------------------------------------------------
\subsection{Q3c --- Why Max Pooling Gives Translation Invariance}
% -------------------------------------------------------------------

In the original architecture, $Y = V \cdot A_{\text{flat}} + c$ depends on
the \emph{spatial positions} of activations. Shifting the input moves a strong
activation from one position to another, changing which weight in $V$ it gets
multiplied by, and therefore changing $Y$.

With max pooling, $Y = \max(A)$ depends only on the \emph{value} of the
largest activation, not where it sits in the feature map. So as long as a
small spatial shift preserves which patch fires most strongly --- i.e.\ the
maximum response stays the same --- the output $Y$ and the loss $L$ are
unchanged. Mathematically, if a shift maps $A \to A'$ but
$\max(A') = \max(A)$, then $L$ is identical for both inputs.

This is the key advantage of global max pooling for classification: moderate
translations that do not destroy the dominant feature response leave the
prediction completely unaffected.

\newpage

\vspace*{\fill}
\begin{center}
    \Huge{Programming Section}
\end{center}
\vspace*{\fill}
\newpage
% ===================================================================
...
\begin{subfigure}{0.48\textwidth}
  \includegraphics[width=\linewidth]{plots/mnist_loss.png}
  \caption{Loss vs.\ epochs}
\end{subfigure}
\hfill
\begin{subfigure}{0.48\textwidth}
  \includegraphics[width=\linewidth]{plots/mnist_accuracy.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\linewidth]{plots/mnist_filters.png}
...
\centering
\begin{subfigure}{0.48\textwidth}
  \includegraphics[width=\linewidth]{plots/cmnist_loss.png}
  \caption{Loss vs.\ epochs}
\end{subfigure}
\hfill
\begin{subfigure}{0.48\textwidth}
  \includegraphics[width=\linewidth]{plots/cmnist_accuracy.png}
  \caption{Accuracy vs.\ epochs}
\end{subfigure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\linewidth]{plots/cmnist_samples.png}
...
\centering
\begin{subfigure}{0.48\textwidth}
  \includegraphics[width=\linewidth]{plots/stl10_loss.png}
  \caption{Loss vs.\ epochs}
\end{subfigure}
\hfill
\begin{subfigure}{0.48\textwidth}
  \includegraphics[width=\linewidth]{plots/stl10_accuracy.png}
  \caption{Accuracy vs.\ epochs}
\end{subfigure}
...
\centering
\begin{subfigure}{0.48\textwidth}
  \includegraphics[width=\linewidth]{plots/gradcam_correct_1.png}
  \caption{Correct prediction 1 (airplane)}
\end{subfigure}
\hfill
\begin{subfigure}{0.48\textwidth}
  \includegraphics[width=\linewidth]{plots/gradcam_correct_2.png}
...
\centering
\begin{subfigure}{0.48\textwidth}
  \includegraphics[width=\linewidth]{plots/gradcam_wrong_1.png}
  \caption{Incorrect prediction 1 (true: airplane)}
\end{subfigure}
\hfill
\begin{subfigure}{0.48\textwidth}
  \includegraphics[width=\linewidth]{plots/gradcam_wrong_2.png}
